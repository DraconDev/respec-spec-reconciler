...
export class LoopController {
	...
	async start(specPath: string, ctx: ExtensionContext): Promise<void> {
		...
		// Update UI
		ctx.ui.setStatus("respec", `🔄 Reconcile: ${nextTarget}`);
		ctx.ui.setWorkingMessage(`[respec] Fixing: ${nextTarget}`);
	...
	async onAgentEnd(
		...
		state.status = "paused";
		setStore(state);
		ctx.ui.setStatus("respec", `⏸ Paused: ${state.currentRound} rounds completed`);
	...
	private showEscapeValveMessage(ctx: ExtensionContext, state: RespecState): void {
		...
		let message = `⚠️ Blocked: ${state.escapeValve.invariant}`;
...
`;
		...
		ctx.ui.setStatus("respec", `❌ Blocked: ${state.escapeValve?.invariant}`);
		ctx.ui.setWorkingMessage();
	}
	...
	private async notifySuccess(ctx: ExtensionContext, state: RespecState): Promise<void> {
		...
		ctx.ui.setStatus("respec", `✅ Reconciled: ${state.specKey.split("/").pop()}`);
		ctx.ui.setWorkingMessage();
	}
}